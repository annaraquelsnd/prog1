...
void OrdenaCandidatos(tCandidato * vet, int qtd);
int CandidatoAehMelhorQueCandidatoB(tCandidato a, tCandidato b);
int MelhorCandidatoEntreAeB(tCandidato * vet, int a, int b);
...
int main(){
    ...
    OrdenaCandidatos(candidatos, qtdCand);
...
void OrdenaCandidatos(tCandidato * vet, int qtd){
    int candidato = 0, indexMelhorCandidato = 0;
    
    tCandidato aux;

    for (candidato = 0; candidato < qtd-1; candidato++) {
        indexMelhorCandidato = MelhorCandidatoEntreAeB(vet, candidato+1, qtd);
        if (CandidatoAehMelhorQueCandidatoB(vet[indexMelhorCandidato],vet[candidato])) {
            aux = vet[candidato];
            vet[candidato] = vet[indexMelhorCandidato];
            vet[indexMelhorCandidato] = aux;
        }
...
int MelhorCandidatoEntreAeB(tCandidato * vet, int a, int b){
    ...
    a++;
    while (a < b) {
        if(CandidatoAehMelhorQueCandidatoB(vet[a], vet[indexMelhor])){
            indexMelhor = a;
        }
        a++;
    }
